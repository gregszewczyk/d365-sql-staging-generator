...
export const TypeBlockCard: React.FC<TypeBlockCardProps> = (props) => {
  ...
  return (
    <div className="ossfb-block" style={{ borderLeftColor: accent }}>
      <div className="ossfb-block-header">
        <span className="ossfb-type-pill" style={{ backgroundColor: accent }}>
          {typeDef ? typeDef.label : "Choose type"}
        </span>
        <select
          className="ossfb-select ossfb-type-select"
      ...
      {block.objectType === "" ? (
        <div className="ossfb-hint">Pick a source object type to start adding conditions.</div>
      ) : (
        <GroupPanel
          group={block.root}
